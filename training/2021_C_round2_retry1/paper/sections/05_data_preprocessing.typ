...
题面中损耗率为 0 表示当周未由该转运商承运，并非零损耗运输。因此，损耗率只在正记录上取均值。T1 至 T8 的预测损耗率依次为 1.90%、0.92%、0.19%、1.57%、2.89%、0.54%、2.08% 和 1.01%。正值原料量采用 $Q_3 + 3 I Q R$ 规则筛查，阈值为 #locked.outlier_threshold.display m³，共标记 #locked.outlier_count.display 个记录。大额供货可能恰是头部供应商能力的证据，故不截尾、不平滑；其未来可重复性留作模型边界。
