...
问题二相对采购成本复算为 491,125.620530，问题三 24 周 C 类原料复算为 69,507.840621 m³，问题四 24 周产品等价到货复算为 800,062.211085 m³，均与报告值零误差。附件 A、B 回读逐格比较正式决策与写入值；LibreOffice 重算后无公式错误或空缓存。14 项故障覆盖负订货、能力超限、库存守恒篡改、转运超载、多承运商、损耗和目标篡改等，全部由预期检查器拒绝。

== Python 与 MATLAB 交叉验证

本文以 Python/HiGHS 作为正式优化实现，并使用 MATLAB 对供应商评价、词典序规划、目标值和硬约束进行独立复现。两种实现的主要目标值和聚合决策在预设容差内一致。MATLAB 直接读取官方 Excel，采用 `intlinprog` 求解问题二、`linprog` 求解问题三和问题四；它不调用 Python，也不把 Python 中间结果作为优化输入。前 10、前 50 排名完全一致，评分最大误差为 $1.11 times 10^(-16)$，全部连续量的最大跨语言误差为 $1.60 times 10^(-10)$，两端硬约束违约均为 0。具体版本、命令和退出状态见附录 E、F。
