...
Python 正式求解采用 SciPy/HiGHS 的 MILP/LP 接口 [8-11]。独立检查器不导入求解器模块，只读取官方材料、锁定参数、原始决策变量和输出表，重新计算供应能力、订单响应、运输损耗、到货换算、库存与目标值。这样可减少求解器与验证器使用同一错误公式的风险。
...
#source-note[验证检验的是决策是否满足本文定义的模型。价格、仓容和供应中断概率仍需要结合现场信息核验。]
