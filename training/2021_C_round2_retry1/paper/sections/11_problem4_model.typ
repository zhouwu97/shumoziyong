...
@eq-capacity-objective 最大化稳定周流量，@eq-capacity-inventory 则避免把一次性初始库存释放误认为可持续产能。平稳方案满足 $A_t=D$，库存维持在 $2D$；若仍固定使用基准安全库存 #locked.initial_inventory.display m³，就会低估产能提高后的缓冲需求。因此，该目标是企业已提前建立 $2D$ 安全库存后的确定性稳态上界；从现有库存立即实施时还需单独安排过渡期。
