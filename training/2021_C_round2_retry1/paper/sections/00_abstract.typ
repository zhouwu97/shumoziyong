...
问题一由产品等价供货能力、按单供货概率、稳定性和兑现率形成业务型加权评分；小幅单项权重扰动下前 50 名重合率不低于 98%，但等权时 S140 从第 #locked.s140_base_rank.display 名降至第 #locked.s140_equal_rank.display 名，说明排序依赖业务偏好。问题二先闭合最少供应商数的上下界：能力最强的前 25 家仅覆盖 #locked.problem2_top25_net_capacity.display m³/周，低于 #locked.base_weekly_production.display m³/周需求，而 #locked.problem2_minimum_supplier_count.display 家正式模型可行；随后在全部 #locked.supplier_count.display 家候选中只锁定供应商基数值，以相对采购成本和运输损耗为后续词典序目标，求得单承运商方案，周成本为 #locked.problem2_weekly_cost.display，周损耗为 #locked.problem2_weekly_loss.display m³。
...
压力测试中，需求增加 10%时已找到可行解但未证明成本最优；关键供应商能力下降 10%和全期中断情景均得到全局最优可行解，供应商数分别为 26 和 27。上述结论限定在历史统计与平稳参数假设内，不构成未来供货或长期产能保证。
