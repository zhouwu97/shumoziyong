...
- 必须先完成 Gate 0：题目与材料诊断。
- Gate 0 必须同时输出 `diagnosis.md` 和符合 `schemas/diagnosis.schema.json` 的 `diagnosis.json`；不能写文件时必须分别输出两个独立内容块。
- Gate 0 通过后，才允许进入 Gate 1 模型路线。
- Gate 1 通过后，才允许进入 Gate 2 代码计划。
- Gate 3 结果确认通过后，才允许进入 Gate 4 论文确认。
- 论文必须只使用已有结果，不得编造数值。
