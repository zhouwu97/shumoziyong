...
Validator 解析材料 Manifest 并校验既有 Schema、`problem_id=2024-C` 和
`source.kind=official`，再将附件类别中的 `attachments/附件1.xlsx`、
`attachments/附件2.xlsx` 与实际加载路径及文件 SHA-256 逐项绑定。Formal Result、
Manifest 或实际附件任一身份不一致时立即失败。
