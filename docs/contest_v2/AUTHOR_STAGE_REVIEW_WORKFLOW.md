...
→ R1 MODEL_REVIEW（编码前）
→ 编码、实验与问级结果
→ R2 EXPERIMENT_REVIEW
→ 论文初稿
→ R3 PAPER_COHERENCE_REVIEW
→ Verification / Ledger / PDF / package-draft
→ R4 FORMAT_SUBMISSION_REVIEW
→ Paper Admission
→ 编排器生成 review_request.json
→ 配置 adapter 后创建 R5 独立 Reviewer 对话
```
...
Paper Admission 通过且 R1--R4 没有未处理的否决项后，由编排器生成 `review/review_request.json` 并进入
`REQUEST_READY`。只有配置外部 Reviewer adapter 后，`dispatch` 才创建独立 Reviewer 对话并保存
`task_id`；未配置时不得声称对话已创建。R5 不预设局部范围，自主全面检查模型、实验、论文、格式、提交包、
创新性、可读性和 AI 披露。修补 MUST 后重新运行受影响的工程产物，再用 `rereview` 生成新的复审请求。
不得在作者对话中自评，也不要求 Reviewer 计算或比较 PDF、学习上下文哈希。`contest` 薄 CLI 只负责生产链，
编排入口为 `scripts/contest_review_orchestrator.py`。
