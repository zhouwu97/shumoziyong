"""复算五题 full_replay 证据并派生 Competition Production 生命周期。"""
...
CONTRACT_PATH = ROOT / "runtime_contracts" / "competition_full_replay_campaign_v1.json"
...
def evaluate_campaign(
    ...
    """验证固定五题，并仅在五题全部通过时派生 full_replay_passed。"""
    contract = _load_object(CONTRACT_PATH, "competition_full_replay_campaign_v1.json")
    _validate_schema(contract, "competition_full_replay_campaign.schema.json", "Campaign 合同")
    manifest = _load_object(manifest_path.resolve(), "Campaign manifest")
    _validate_schema(manifest, "competition_full_replay_manifest.schema.json", "Campaign manifest")
    ...
    report = {
        ...
        "artifact_type": "competition_full_replay_campaign_report_v1",
        ...
        "derived_lifecycle": "full_replay_passed" if campaign_passed else "review_ready",
        "new_problem_default_enabled": False,
    }
    _validate_schema(report, "competition_full_replay_report.schema.json", "Campaign report")
    return report
