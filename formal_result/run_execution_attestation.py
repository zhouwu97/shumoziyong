...
def verify_run_execution_attestation(
    ...
    *,
    ...
    for field in (
        ...
        "targeted_host_read_controls_passed",
    ...
    if record.get("execution_trust_model") != EXECUTION_TRUST_MODEL:
        raise FormalResultVerificationError("Run 未声明 trusted_local 执行信任模型")
    if record.get("git_state_clean") is not True:
        raise FormalResultVerificationError("trusted-local 执行前 Git 状态不干净")
    if record.get("git_head") != derivation_summary["collector_source_commit"]:
        raise FormalResultVerificationError("Git HEAD 未绑定实际 Collector source commit")
    git_state = record.get("git_state")
    if not isinstance(git_state, Mapping) or not (
        git_state.get("git_head") == record.get("git_head")
        and git_state.get("git_state_clean") is True
        and git_state.get("status_porcelain_v1") == ""
        and git_state.get("status_exit_code") == 0
        and git_state.get("diff_exit_code") == 0
        and git_state.get("diff_cached_exit_code") == 0
    ):
        raise FormalResultVerificationError("Git HEAD/clean-state 证明不完整")
    if record.get("default_deny_host_reads_verified") is not False:
        raise FormalResultVerificationError("trusted-local 不得宣称已验证全局默认拒绝读取")
    ...
    if not required_policy.issubset(set(record.get("sandbox_policy_settings", []))):
        raise FormalResultVerificationError("Sandboxie 策略未形成 targeted host-read controls")
    ...
    if record.get("targeted_host_read_controls_passed") is not True:
        raise FormalResultVerificationError("targeted host-read controls 未声明通过")
